...
var ErrBufferTooSmall = errors.New("exiftool_v1's buffer too small (see Buffer init option)")

// Exiftool is the exiftool_v1 utility wrapper
...
func NewExiftool(opts ...func(*Exiftool) error) (*Exiftool, error) {
	...
	for _, opt := range opts {
		if err := opt(&e); err != nil {
			return nil, fmt.Errorf("error when configuring exiftool_v1: %w", err)
		}
	}
...
func (e *Exiftool) Close() error {
	...
	go func() {
		if e.cmd != nil {
			if err := e.cmd.Wait(); err != nil {
				errs = append(errs, fmt.Errorf("error while waiting for exiftool_v1 to exit: %w", err))
			}
		}
		ch <- struct{}{}
		close(ch)
	...
	if len(errs) > 0 {
		return fmt.Errorf("error while closing exiftool_v1: %v", errs)
	}
...
// SetExiftoolBinaryPath sets exiftool_v1's binary path. When not specified, the binary will have to be in $PATH
// Sample :
//
//	e, err := NewExiftool(SetExiftoolBinaryPath("/usr/bin/exiftool_v1"))
