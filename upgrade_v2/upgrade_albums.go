package main

import (
	"encoding/json"
	"fmt"
	"github.com/google/uuid"
	"log"
	"os"
	"path/filepath"
	"time"
)

func upgradeAlbums() ([]*AlbumV1, error) {

	const directoryName = "albums"

	itemFile := filepath.Join(metadatasDir, currentVersion, "albums.json")

	// Read V1 data
	data, err := os.ReadFile(itemFile)
	if err != nil {
		return nil, fmt.Errorf("read failed: %w", err)
	}

	var itemArrayV1 []*AlbumV1
	if err = json.Unmarshal(data, &itemArrayV1); err != nil {
		return nil, fmt.Errorf("unmarshal failed: %w", err)
	}

	for _, item := range itemArrayV1 {

		u7, err2 := uuid.NewV7()
		if err2 != nil {
			return nil, fmt.Errorf("error generating UUIDv7: %w", err)
		}
		item.NewID = u7.String()
	}

	// Convert to V2 with pointers
	itemArrayV2 := make([]*AlbumV2, len(itemArrayV1))
	var index = 1
	for i := range itemArrayV1 {
		// Initialize destination pointer
		itemArrayV2[i] = &AlbumV2{}

		// Copy matching fields
		copyMatchingFields(&itemArrayV2[i], &itemArrayV1[i])

		// Set new UUID-based ID
		itemArrayV2[i].ID = itemArrayV1[i].NewID
		itemArrayV2[i].Number = index
		itemArrayV2[i].Title = "444"
		itemArrayV2[i].Type = itemArrayV1[i].AlbumType
		itemArrayV2[i].IsCollectionValid = itemArrayV1[i].IsCollection

		itemArrayV2[i].LastSeen = time.Now()

		itemArrayV2[i].CreatedAt = itemArrayV1[i].CreationDate
		itemArrayV2[i].UpdatedAt = time.Now()
		itemArrayV2[i].Version = "2"
		index++
	}

	m := filepath.Join(metadatasDir, newVersion, directoryName)
	err = CreateDirectory(m)
	if err != nil {
		return nil, err
	}

	for _, item := range itemArrayV2 {
		err = WriteData(filepath.Join(m, item.ID+".json"), &item)
		if err != nil {
			return nil, err
		}
	}

	log.Printf("Upgraded %d albums", len(itemArrayV2))
	return itemArrayV1, nil
}
